...
select ceiling(AVG(Age)) as RoundedAverAge from Employees
select * from Employees
...
select * from Sales
select CustomerID, sum(SaleAmount) TotalSales from Sales
group by CustomerID
...
select * from Sales
select ProductID, sum(SaleAmount) TotalSales, avg(SaleAmount) AvgSales from Sales
group by ProductID
...
select DepartmentName, avg(Salary) AvgSalary, count(EmployeeID) NumEmplo from Employees
group by DepartmentName
...
select * from Sales
select Year(SaleDate), sum(SaleAmount) TotalSales from Sales
...
select * from Employees
select DepartmentName, sum(Salary) TotalSalary, avg(Salary) AvgSalary from Employees
group by DepartmentName
...
select Year(OrderDate) as orderYear, min(TotalAmount) MinAmount, max(TotalAmount) MaxAmount from Orders
